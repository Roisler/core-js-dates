...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const millisecondsOfDay = 86_400_000;
  const countMillisecondOnPeriod = new Date(dateEnd) - new Date(dateStart);
  return countMillisecondOnPeriod / millisecondsOfDay + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let countWeekensInMonth = 0;
  for (let i = 1; i <= countDays; i += 1) {
    const day = date.getDay();
    if (day === 0 || day === 6) {
      countWeekensInMonth += 1;
    }
    date.setDate(date.getDate() + 1);
  }
  return countWeekensInMonth;
}
...
function getWeekNumberByDate(date) {
  const startYearDate = new Date(date);
  startYearDate.setMonth(0, 1);
  const firstDayOfYear = startYearDate.getDay();

  const countMillisecondOnDay = 86_400_000;
  const countDaysOnPeriod =
    (date - startYearDate) / countMillisecondOnDay + firstDayOfYear + 1;

  const weekCount = Math.ceil(countDaysOnPeriod / 7);
  return weekCount;
}
